...
class TreeVisibility:
    ...
    def _is_visible_vertically(self, tree:int, tree_grid:list[str], position:tuple()) -> bool:
        position = (position[1],position[0])
        return self._is_visible_horizontally(tree, tree_grid,position)
    ...
    def get_number_of_visible_trees(self, input:str) -> int:
        ...
        for idx_v, line in enumerate(self.tree_grid [1:-1]):
            for idx_h, tree in enumerate(line[1:-1]):
                ...
                #mira izqd
                #mira derecha
                #si alguno es true, no continues
